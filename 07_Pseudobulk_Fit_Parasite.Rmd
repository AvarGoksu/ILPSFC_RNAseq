...
coldata <- coldata %>% 
  dplyr::mutate(leiden_anno_grouped = 
                ...
                infection_status =
                  case_when(stage_grouped != "uninfected" ~ "yes",
                            .default = "no")) %>%
  dplyr::group_by(condition) %>%
  dplyr::mutate(subsample_size = ceiling(n()/10)) %>%
  dplyr::ungroup() #Here the subsample_size will show the appropriate number of subsample sizes for 5-fold random subsampling
```
